...
async function fetchDebtorData(id: string): Promise<BCRALookupState> {
  const response = await fetch(`/api/central-de-deudores/${id}`, {
    cache: "no-store",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`Debtor lookup returned ${response.status}`);
  }

  const data = (await response.json()) as Omit<BCRALookupState, "loading">;
  return {
    ...data,
    loading: false,
  };
...
export function DebtorClientPage({ id }: { id: string }) {
  const [state, setState] = useState<BCRALookupState>({
    deudaData: null,
    historialData: null,
    chequesData: null,
    unavailable: {
  ...
  useEffect(() => {
    ...
    async function loadBCRAData() {
      ...
      try {
        const data = await fetchDebtorData(id);
        if (cancelled) return;
  ...
  const { deudaData, historialData, chequesData, unavailable, loading } = state;
  const hasData = deudaData || historialData || chequesData;
  const hasCriticalUnavailableData =
    unavailable.deudas ||
    unavailable.historial ||
    (!hasData && unavailable.cheques);
  const denominacion =
      ...
      {loading ? (
        <Card className="mb-6">
          <CardContent className="flex items-center gap-3 p-6 text-muted-foreground">
            ...
            Consultando información en el BCRA...
          </CardContent>
        </Card>
      ) : (
        <>
          {hasCriticalUnavailableData && (
            <Alert variant="destructive" className="mb-6">
              <AlertTriangle className="h-4 w-4" />
              <AlertTitle>El BCRA no respondió la consulta</AlertTitle>
              <AlertDescription>
                No se pudo obtener la información principal desde el BCRA. Probá
                de nuevo en unos minutos.
              </AlertDescription>
            </Alert>
          )}

          {!hasData && hasCriticalUnavailableData ? (
            <Card className="mb-6">
              <CardHeader>
                ...
                <CardDescription>
                  No se pudo obtener la información desde el BCRA en este
                  momento.
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <div className="grid gap-6">
              ...
              {unavailable.cheques ? (
                <Alert className="mb-0">
                  ...
                  <AlertDescription>
                    No se pudo consultar la central de cheques rechazados en
                    este momento.
                  </AlertDescription>
                </Alert>
              ) : (
          ...
          {hasData && (
            <div className="mt-8">
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    >
                      INAES. Servicios de Crédito Cooperativo y/o Ayuda
                      Económica Mutual
                    </ClipboardLink>
                  </CardContent>
                </Card>
              ...
              <div className="text-sm text-muted-foreground mb-8">
                <p>
                  El BCRA no tiene responsabilidad alguna por los datos
                  difundidos en los enlaces anteriores. La información
                  corresponde a las respectivas entidades mencionadas.
                </p>
              </div>
              ...
              <p>
                Nombre o razón social de la persona humana o jurídica que figura
                en el padrón de la Agencia de Recaudación y Control Aduanero
                (ARCA) o bien la que fuera registrada por la entidad informante.
              </p>
              ...
              <p>
                Indica la clasificación del deudor informada por la entidad.
                Para más información, acceder al{" "}
                <Link
              ...
              <ul className="list-disc pl-6 mt-2 space-y-2">
                ...
                <li>
                  <strong>Situación 2:</strong> Con seguimiento especial |
                  Cartera comercial y Riesgo bajo | Cartera para consumo o
                  vivienda
                </li>
                <li>
                  <strong>Situación 3:</strong> Con problemas | Cartera
                  comercial y Riesgo medio | Cartera para consumo o vivienda
                </li>
                <li>
                  <strong>Situación 4:</strong> Con alto riesgo de insolvencia |
                  Cartera comercial y Riesgo alto | Cartera para consumo o
                  vivienda
                </li>
                <li>
                  <strong>Situación 5:</strong> Irrecuperable | Cartera
                  comercial y Cartera para consumo o vivienda
                </li>
              </ul>
              ...
              <p>
                ...
                refinanciaciones, recategorización obligatoria o situación
                jurídica. La leyenda (N/A) indica &quot;No Aplicable&quot;.
              </p>
              ...
              <p>
                Los deudores se identifican según la Ley 25.326 de Protección de
                los Datos Personales:
              </p>
              <ul className="list-disc pl-6 mt-2">
                <li>Información sometida a revisión (artículo 16, inciso 6)</li>
                <li>
                  Información sometida a proceso judicial (artículo 38, inciso
                  3)
                </li>
              </ul>
            </div>
