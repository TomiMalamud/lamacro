...
const needsFreshData = (endYear: number, endMonth: number): boolean => {
  const latestDataYear = Math.max(...Object.keys(typedHistoricalInflation).map(Number));
  const latestDataMonth = Math.max(...Object.keys(typedHistoricalInflation[latestDataYear.toString()] || {}).map(Number));
  
  // We need fresh data if requesting data beyond what's in the historical JSON
  return (endYear > latestDataYear) || 
...
export const useInflationData = () => {
  const [combinedData, setCombinedData] = useState<InflationRates>(typedHistoricalInflation);
...
const InflationCalculator = (props: InflationCalculatorProps): InflationResult => {
  ...
  const needsFetch = needsFreshData(props.endYear, props.endMonth) && 
...
export const InflationCalculatorComponent = (props: InflationCalculatorProps): InflationResult => {
