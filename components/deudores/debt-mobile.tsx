...
export default function DebtMobileSection({ deudaData }: { deudaData: DeudaResponse | null }) {
                        ...
                        <div key={index} className={`py-4 ${index === 0 ? '' : 'border-t'}`}>
                            <div className="space-y-3">
                                <div className="flex flex-col gap-2">
                                    <div className="font-semibold text-base">{entidad.entidad}</div>
                                    <div className="flex items-center gap-2">
                                        <DebtBadge
                                            label={getSituacionDescription(entidad.situacion)}
                                            colorClass={getSituacionColor(entidad.situacion)}
                                        />
                                        <span className="text-muted-foreground">Días de atraso: {entidad.diasAtrasoPago ?? "N/A"}</span>
                                    </div>
                                    <div>
                                        <span className="text-muted-foreground">
                                            Deuda: {" "}
                                            <span className="text-foreground">{formatCurrency(entidad.monto)}</span>
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {entidad.refinanciaciones && (
                                        <DebtBadge label="Refinanciado" colorClass="bg-blue-100 text-blue-800" />
                                    )}
                                    {entidad.recategorizacionOblig && (
                                        <DebtBadge label="Recategorizado" colorClass="bg-orange-100 text-orange-800" />
                                    )}
                                    {entidad.situacionJuridica && (
                                        <DebtBadge label="Situación Jurídica" colorClass="bg-purple-100 text-purple-800" />
                                    )}
                                    {entidad.irrecDisposicionTecnica && (
                                        <DebtBadge label="Irrecuperable DT" colorClass="bg-red-100 text-red-800" />
                                    )}
                                    {entidad.enRevision && (
                                        <DebtBadge label="En Revisión" colorClass="bg-yellow-100 text-yellow-800" />
                                    )}
                                    {entidad.procesoJud && (
                                        <DebtBadge label="Proceso Judicial" colorClass="bg-red-100 text-red-800" />
                                    )}
                                </div>
                            </div>
                        </div>
